...
mixing/separation in the context of metallurgical processes \cite{johansen1988fluid,metal},  
conventional modes of heat transfer \cite{deckwer1980mechanism,bubble}
and ever so importantly, the transmission of pathogens \cite{lydia_1,lydia_2}. 
...
the present body of work is divided into two parts :  
...
our understanding of the distribution of drop sizes, as there is no meaningful way 
to distinguish between drops generated via physically consistent processes and 
those produced as a result of cascading numerical errors. 


% demonstrate the effect of such errors, raindrop, 

Ever since such numerical instabilities were first observed with 
the ``SURFER'' \cite{lafaurie1994modelling} code
in the context of planar jets and rising bubbles, 
considerable efforts have been made towards the design of numerical methods 
to specifically deal with flows involving such marked density contrasts
...
\marginnote{
	A difference in the drop size distributions obtained via \textit{consistent} 
	and non-consistent methods has been observed by Yue Ling and coworkers 
	\cite{stanley_exp,stanley_lognormal} in the context of planar atomizing mixing layers. 
	Use of the non-consistent method yielded in an exponential distribution for 
	the large sizes, whereas with the consistent method they recovered a Log-Normal distribution. 
	}
...
They are commonly modeled as singular source terms in the momentum balance equation that 
...
\caption{Numerical simulations of a $3 mm$ raindrop falling in air under gravitational acceleration.
	...
	enables us to suppress the massive interfacial deformations of the drop, 
...
thereby enabling a `natural' and intuitive 
...
applicability only to 2D in the case of Raessi and Pitsch, 
...
Sticking to the breakup of liquid sheets, another important transition involves the  
...
The development of quantitative models geared towards predicting the statistical properties 
...
have led to the popularization of three distinct classes of probability density functions, 
...
Even though the aforementioned models have been applied to a wide variety of flow configurations with
varying degrees of success, there is a general lack of consensus regarding their validity, primarily 
...
succinctly characterized by a point in the phase space of Weber and Ohnesorge numbers.  
...
of the second kind (\cite{barenblatt}) due to their dependence on an external length scale.
Finally, at moments very close to the pinch-off, neither of the above regimes 
are valid as the thickness of the thread approaches the intrinsic length scale (corresponding to $\textrm{Oh} = 1$) 
...
\begin{itemize}
	...
	\item Chapter 5 provides a thorough description of our numerical setup for the spatially periodic
		ligaments, the methodology behind the generation of randomly perturbed (corrugated) surfaces 
