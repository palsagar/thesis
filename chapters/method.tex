...
\begin{align}
   L = L_{adv} + L_{\mu} + L_{\sigma} + L_{g} 
   \label{opr}
\end{align}

where the $L_{adv}$ represents the conservative advection, 
...
operator $L^{h}_{vof}$ is in accordance with the Lagrangian explicit or Eulerian implicit 
...
The advection operator $L^{h}_{adv}$ is implemented as high-order spatial schemes 
...
The functionality of the operator $L^{h}_{adv}$ near the interface is tighly coupled
to that of $L^{h}_{vof}$ from equation \ref{cvof_update}, 
