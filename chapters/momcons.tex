...
{\cijk^{n+1} - \cijk^{n}} = - \sum_{\textrm{faces} \, f} F^{(c)}_f + \int_{t_n}^{t_{n+1}}  
	{\textrm{d}}t \int_\Omega  H \,\nabla \cdot \boldsymbol{u}\,  {\textrm {d}}\X   \,,
\label{sumf}
\end{align}
...
Consider the advection of a generic \textit{conserved} scalar quantity $\phi$ by a continuous velocity field
\begin{align}
...
is verified for the density $\rho$, the momentum $\rho \boldsymbol{u}$ or the internal energy 
...
\begin{align}
F_f^{(\phi)} = 
\int_{t_n}^{t_{n+1}} {\textrm d}t \int_{f} [ u_f  \,H \,\phi  +  u_f \,(1-H) \,\phi ] 
\, {\textrm d}\X \,, 
\label{fluxphi}
\end{align}
