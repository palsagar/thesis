...
Compared to the shifted fractions method in the previous section, the presence of the 
sub-grid volume fraction field facilitates a more `natural' computation of not only 
the staggered volume fraction field $C^{n+1}_q$, but more importantly its fluxes. 
...
The primary motivation regarding the development of this method is the issue of 
conservation (lack of) concerning the discrete transport of momentum in case of
the shifted fractions method.    

\subsubsection*{Spatial Configuration : Coarse \& Sub-Grid Variables}

\begin{figure}[h!]
\includegraphics[width = \textwidth]{plots/grid.png} 
\centering
\caption{Arrangement of primary variables on the coarse and fine grids. The figure on the left corresponds to the coarse grid, and the one on the right corresponds to the fine grid arrangement.}
\label{sub_grid}
\end{figure}

At the coarse grid level, the pressure and velocity fields are defined in a staggered configuration, with pressures at the cell centers (centroids) and velocities on the cell face centers. Fig. \ref{grid} demonstrates the arrangement of variables, which one can easily extrapolate to 3D during implementation, but for the sake of clarity we illustrate its 2D equivalent. The volume fraction $C$ (labeled $c$ in the figure) is defined on a grid that is twice as fine than that of the pressure/velocity grid, we shall henceforth refer to it as the fine grid. Therefore in 3D, each cubic control volume is divided into eight constituent smaller cubic volumes at the centroids of which, the volume fraction field is centered. 
